...
async function SearchStudents(req, res) {
  ...
  try {
    ...
    if (Filters.Class && Filters.Class != "All") {
      ClassCondition.Class = Filters.Class;
    }
    ...
    if (SearchMode == "Name") {
      ...
      students = await Students.find(query).select(
        "LastName FirstName fatherName DOA contact Section GRNO RollNo Class WA email"
      );
    }
    res.json({
      success: true,
...
async function FilterStudents(req, res) {
  ...
  try {
    ...
    let ClassCondition = {};
    if (Filters.Class && Filters.Class != "All") {
      ClassCondition.CurrentClass = Filters.Class;
    }
