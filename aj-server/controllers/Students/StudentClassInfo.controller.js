...
const getStdClassDetails = async(req, res) => {
  const Class = await Students.findById(req.details.Student)
    ...
    const Payload = {class_name:result.CurrentClass.name,section_name:result.CurrentSection.name,start_date:result.CurrentSection.start_date,ClassTeacher:result.CurrentSection.ClassTeacher,Students:result.CurrentSection.Students.length}
  res.json({
