...
const Users = (await User.find({_id:{$ne:req.AdminId},Role:{$ne:"chief admin"}})).map(user=>({...user._doc,isStaff:user.StaffId?true:false}))
...
const CreateUserAccount = async(req,res)=>{
    ...
    bcrypt.hash(payload.password, 8, async function (err, hash) {
        if(!err) await User.create({...payload,password:hash})
        else res.status(501).json({success:false,message:"An error occured . Try again later."})
    })
...
const GetUserInfo  = async(req,res)=>{
    let {id} = req.params
    if(!id||id.length!=24) return res.status(404).json({success:false,message:"Invlaid Id"})
    let user = await User.findById(id).select("-password -logOutRequired -isBlocked -photo -LastLogin")
...
const UpdateUserInfo  = async(req,res)=>{
    ...
    try{
        ...
        bcrypt.hash(payload.password, 8, async function (err, hash) {
            if(!err) await User.findByIdAndUpdate(id,{...payload,password:hash})
            else return res.status(501).json({success:false,message:"An error occured. Try again later."})
    })
}
...
module.exports = {getUsers,ToggleGlobalRestriction,ToggleBlockIndividualUser,CreateUserAccount,GetUserInfo,UpdateUserInfo,DeleteUserAccount}
