...
import { IfilterableStats, IfilterableStatsPayload } from "../Types/IfilterableStats";
...
export const InsertFiltersFn = (state:IfilterableStats , {payload:{available,selected,isLoading}}:PayloadAction<Ifilters>)=>{
    if(isLoading!=undefined) state.isLoading = isLoading
        ...
        if(selected){
        let{Class,PaymentConfig,Session,month,year,feeFrequency}= selected
...
export const InsertPayloadFn = (state:IfilterableStats , {payload:{payload,isLoading}}:PayloadAction<Ipayload>)=>{
