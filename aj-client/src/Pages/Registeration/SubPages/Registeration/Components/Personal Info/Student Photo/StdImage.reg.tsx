...
const RegStdImage = () => {
  ...
  const [{ Error, Preview, isCapture }, setFileState] = useState<IstdImageFile>(
    {
      Error: "",
      ImageFile: null,
      Preview: getValues("photo"),
      isCapture: false,
    }
  );
  ...
  return (
    <section className="w-[18%] px-2 max-lg:w-full flex flex-col gap-y-2 ">
      ...
      <WebCamImage setFileState={setFileState} isCapture={isCapture} />{" "}
      {/* //! To access the image from webcam  */}
      <div
        {...getRootProps({
          className: ` dropzone lg:w-full max-lg:w-[20%]  max-md:w-[25%] max-sm:w-[35%] bg-box aspect-square border-[var(--darker)] 
...
const CaptureButton: FC<{
  setFileState: Dispatch<SetStateAction<IstdImageFile>>;
  isCapture: boolean;
}> = ({ setFileState, isCapture }) => {
  const FalseCapture = () => setFileState((e) => ({ ...e, isCapture: false }));
  const HandleCancelCapture = () =>
    setFileState({ Error: "", ImageFile: null, Preview: "", isCapture: false });
    ...
    return (
      <>
        <Button
          type="button"
          onClick={() =>
            setFileState((e) => ({ ...e, isCapture: !e.isCapture }))
          }
          className={clsx(
