  ...
  return (
    <section className="flex gap-4 overflow-auto customscrollbarTable pb-4">
      ...
      <RoleBasedAccess roleToGiveAccess={"chief admin"}>
            ...
            <Link to={"users"} className={`${ValidateRoute({toCompare:"/settings/users",classesToApply:ActiveStyle})} px-4 dark:bg-gray-900 bg-gray-200 text-nowrap whitespace-nowrap text-sm  hFont font-medium text-gray-700 dark:text-white rounded-md py-1 `}>Users Control</Link>
            </RoleBasedAccess>
            <RoleBasedAccess roleToGiveAccess={"chief admin"}>
            <Link to={"new-account"} className={`${ValidateRoute({toCompare:"/settings/new-account",classesToApply:ActiveStyle})} px-4 dark:bg-gray-900 bg-gray-200 text-nowrap whitespace-nowrap text-sm  hFont font-medium text-gray-700 dark:text-white rounded-md py-1 `}>New Users</Link>
            </RoleBasedAccess>
            <RoleBasedAccess roleToGiveAccess={"chief admin"}>

            <Link to={"advanced-actions"}
             className={`${ValidateRoute({toCompare:"/settings/advanced-actions",classesToApply:ActiveStyle})} px-4 dark:bg-gray-900 bg-gray-200 text-nowrap whitespace-nowrap text-sm  hFont font-medium text-gray-700 dark:text-white rounded-md py-1 `}>
