...
const StudentsTable = () => {
  const { isLoading, MutableData, isNotFound } = useAppSelector(
    (state) => state.StudentsDir
  );
  return (
    <>
      <Table className="bg-[var(--box)] dark:bg-dark dark:text-white rounded-lg shadow">
        <StudentsTableHead />
        <TableBody>
          {MutableData?.map((data) => (
            <StudentsTableRow data={data} />
          ))}
        </TableBody>
      </Table>
      <div className="w-full">
        {isLoading ? (
          ...
          (isNotFound || MutableData.length == 0) && (
            <div className="center">no students found</div>
          )
        )}
