  ...
  return (
    <TableRow >
        ...
        <TableCell className='flex gap-x-4 justify-start'>
            ...
            <a  href={data.WA?`https://wa.me/+92${data?.WA?.split("92")[1]||data.WA}`:`tel:+92${ 
                 data?.contact[0]?.split("+92")[1] || data.contact[0]}`} target="_blank" className={`p-2 cursor-pointer transition-colors hover:bg-[var(--dark)] hover:text-white bg-[var(--bg)] text-[var(--dark)] rounded-full aspect-square`}>
                {
                    data.WA?
                    <FaWhatsapp  size={18}/>:
                <Phone size={18}/>
        ...
        <Link to={`/students/history/${data._id}`} className="center text-dark dark:text-light">
                <History size={22}/>
                </Link>
                <Link to={`/students/${data.GRNO}`} className="center text-dark dark:text-light">
