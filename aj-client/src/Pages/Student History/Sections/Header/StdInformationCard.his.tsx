...
const StdInformationCard = () => {
  ...
  return (
    <div className="flex justify-between bg-gradient-to-bl text-white to-dark_dimmer   from-dark overflow-hidden relative w-[33%] 
    max-md:w-full rounded-xl shadow-md   gap-2  h-32">
    <div className="flex flex-col gap-1 justify-between  w-full p-4 py-6">
        ...
        <div className="flex justify-end gap-2 font-bold">
            <Link to={`/students/${GRNO}`} className="rounded-md bg-black px-4 py-1 text-white text-sm">Details</Link>
            <button className="rounded-md bg-[var(--box)] px-4 py-1 text-dark text-sm">Add payment</button>
        </div>
    </div>
</div>
  )
